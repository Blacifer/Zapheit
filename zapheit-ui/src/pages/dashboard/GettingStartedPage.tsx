...
export default function GettingStartedPage(props: {
  ...
  const explainGatewayError = (message: string) => {
    ...
    if (text.toLowerCase().includes('provider key missing for openrouter')) {
      return 'Missing OpenRouter runtime key. Set OPENROUTER_API_KEY or the legacy RASI_OPENROUTER_API_KEY in the backend environment (GCP Secret Manager for prod, or zapheit-api/.env for local) and restart/redeploy the backend.';
    }
    if (text.toLowerCase().includes('provider key missing for openai')) {
      return 'Missing OpenAI runtime key. Set OPENAI_API_KEY or the legacy RASI_OPENAI_API_KEY in the backend environment (GCP Secret Manager for prod, or zapheit-api/.env for local) and restart/redeploy the backend.';
    }
    if (text.toLowerCase().includes('provider key missing for anthropic')) {
      return 'Missing Anthropic runtime key. Set ANTHROPIC_API_KEY or the legacy RASI_ANTHROPIC_API_KEY in the backend environment (GCP Secret Manager for prod, or zapheit-api/.env for local) and restart/redeploy the backend.';
    }
