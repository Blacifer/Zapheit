...
interface OutlookDetail extends OutlookMessage {
  cc?: string;
  body?: string;
  isHtml?: boolean;
}
...
function ComposeBox({ mode, email, onClose }: ComposeProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (mode === 'reply' && email?.id) {
        action = 'reply_email';
        data = { id: email.id, body };
      } else if (mode === 'forward' && email?.id) {
        action = 'forward_email';
        data = { id: email.id, to, body };
      } else {
        action = 'send_email';
        data = { to, subject, body };
      }
      const res = await api.unifiedConnectors.executeAction(CONNECTOR_ID, action, data);
      if (res.success) {
...
function EmailDetailView({ emailMeta, onBack, onArchived }: EmailDetailViewProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await api.unifiedConnectors.executeAction(CONNECTOR_ID, 'get_email', { id: emailMeta.id });
  ...
  const handleArchive = async () => {
    setArchiving(true);
    try {
      const res = await api.unifiedConnectors.executeAction(CONNECTOR_ID, 'archive_email', { id: emailMeta.id });
      if (res.success) {
        ...
        {loading ? (
          ...
          <div className="px-5 py-4">
            ...
            <div className="flex items-start gap-3 mb-5">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-600/30 to-indigo-500/30 flex items-center justify-center text-white font-semibold text-sm shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                {(email as OutlookDetail).cc && <p className="text-[11px] text-slate-500">Cc: {(email as OutlookDetail).cc}</p>}
              </div>
            ...
            <div className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap break-words">
              {(detail as OutlookDetail | null)?.body || email.snippet || '(empty)'}
            </div>
          </div>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div>
            {filtered.map((e) => (
              ...
              >
                <div className="relative w-8 h-8 rounded-full bg-gradient-to-br from-blue-600/25 to-indigo-500/25 flex items-center justify-center text-white text-xs font-semibold shrink-0 mt-0.5">
