...
import {
  ...
  Trash2, Zap, AlertCircle, FileText, Database, MessageSquare, AtSign,
} from 'lucide-react';
...
type TriggerType = 'page_created' | 'database_updated' | 'comment_added' | 'mention';
...
const TRIGGER_META: Record<TriggerType, { label: string; description: string; Icon: typeof Bell }> = {
  page_created:      { label: 'Page created',      description: 'Agent processes or enriches new Notion pages',      Icon: FileText },
  database_updated:  { label: 'Database updated',  description: 'Agent reacts to changes in database entries',       Icon: Database },
  comment_added:     { label: 'Comment added',      description: 'Agent responds to new comments on pages',           Icon: MessageSquare },
  mention:           { label: 'Agent mentioned',     description: 'Agent acts when mentioned in a page or comment',    Icon: AtSign },
};

export function NotionAutomationTab() {
  ...
  const [newTriggerType, setNewTriggerType] = useState<TriggerType>('page_created');
  ...
  return (
    <div className="p-5 space-y-8 max-w-2xl">
      ...
      <section className="space-y-3">
        ...
        <div className="relative">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-white/20 pr-8"
            >
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder={'e.g. "Create a new page in Sprint Backlog database for the login bug"'}
            className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-white/20"
            disabled={executing}
          />
          <button
            onClick={() => void handleExecuteCommand()}
            disabled={!command.trim() || !selectedAgent || executing}
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white text-xs font-semibold transition-colors disabled:opacity-30 disabled:cursor-not-allowed shrink-0"
          >
        ...
        <div className="space-y-1">
          ...
          {[
            'Create a new page in the Sprint Backlog database: "Fix login timeout bug"',
            'Search for all pages mentioning "onboarding"',
            'Archive the outdated Q2 planning page',
            'Add a block to the weekly standup page with today\'s updates',
          ].map((ex) => (
            <button
              key={ex}
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Configure agents to automatically respond to Notion events. All actions go through governance.
        </p>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full appearance-none px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-white/20"
              >
            ...
            <div>
              <label className="text-[10px] text-slate-500 uppercase tracking-wider font-medium block mb-1">Database filter (optional)</label>
              <input
                type="text"
                value={newTriggerPipeline}
                onChange={(e) => setNewTriggerPipeline(e.target.value)}
                placeholder="e.g. database ID or leave blank for all"
                className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-white/20"
              />
            </div>
        ...
        {triggers.length === 0 && !showAddTrigger ? (
          <div className="text-center py-8">
            ...
            <p className="text-[10px] text-slate-600 mt-1">Add a trigger to let agents respond to Notion events automatically.</p>
          </div>
        ) : (
              ...
              return (
                <div key={trigger.id} className={cn(
                  ...
                  <button onClick={() => toggleTrigger(trigger.id, 'enabled')} className="text-slate-500 hover:text-white transition-colors">
                    {trigger.enabled ? <ToggleRight className="w-5 h-5 text-white" /> : <ToggleLeft className="w-5 h-5" />}
                  </button>
      ...
      <section className="rounded-xl border border-white/8 bg-white/[0.02] p-4 space-y-2">
        ...
        <p className="text-[11px] text-slate-500 leading-relaxed">
          ...
          governance pipeline. Write actions like <span className="text-slate-400 font-medium">create_page</span>,
          <span className="text-slate-400 font-medium"> update_page</span>, and
          <span className="text-slate-400 font-medium"> archive_page</span> check action policies,
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          ...
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] text-slate-400 font-medium">
            <Shield className="w-3 h-3 text-slate-400" /> Policy check
          </span>
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] text-slate-400 font-medium">
