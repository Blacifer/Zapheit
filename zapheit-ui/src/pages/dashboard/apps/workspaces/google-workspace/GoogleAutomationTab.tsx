...
import {
  ...
  Trash2, Zap, AlertCircle, Mail, Calendar, HardDrive, UserPlus,
} from 'lucide-react';
...
const TRIGGER_META: Record<TriggerType, { label: string; description: string; Icon: typeof Bell }> = {
  email_received:  { label: 'Email received',   description: 'Agent triages or responds to incoming emails',    Icon: Mail },
  event_starting:  { label: 'Event starting',   description: 'Agent prepares meeting briefs automatically',     Icon: Calendar },
  file_shared:     { label: 'File shared',       description: 'Agent monitors and logs file sharing activity',   Icon: HardDrive },
  user_created:    { label: 'User created',      description: 'Agent provisions new employee workspace',         Icon: UserPlus },
};

export function GoogleAutomationTab() {
  ...
  return (
    <div className="p-5 space-y-8 max-w-2xl">
      ...
      <section className="space-y-3">
        ...
        <div className="relative">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500/30 pr-8"
            >
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder={'e.g. "Send an email to team@acme.com about the Q4 update"'}
            className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500/30"
            disabled={executing}
          />
          <button
            onClick={() => void handleExecuteCommand()}
            disabled={!command.trim() || !selectedAgent || executing}
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-400 text-white text-xs font-semibold transition-colors disabled:opacity-30 disabled:cursor-not-allowed shrink-0"
          >
        ...
        <div className="space-y-1">
          ...
          {[
            'Send an email to team@acme.com with the Q4 report summary',
            'Create a calendar event for Friday 2pm: Sprint Planning',
            'Share the onboarding guide with new-hire@acme.com',
            'List all files shared externally in the last 7 days',
          ].map((ex) => (
            <button
              key={ex}
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Configure agents to automatically respond to Google Workspace events. All actions go through governance.
        </p>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full appearance-none px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500/30"
              >
            ...
            <div>
              <label className="text-[10px] text-slate-500 uppercase tracking-wider font-medium block mb-1">Filter (optional)</label>
              <input
                type="text"
                value={newTriggerPipeline}
                onChange={(e) => setNewTriggerPipeline(e.target.value)}
                placeholder="e.g. label:urgent or leave blank for all"
                className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500/30"
              />
            </div>
        ...
        {triggers.length === 0 && !showAddTrigger ? (
          <div className="text-center py-8">
            ...
            <p className="text-[10px] text-slate-600 mt-1">Add a trigger to let agents respond to Google Workspace events automatically.</p>
          </div>
        ) : (
                ...
                <div key={trigger.id} className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500">
                      ...
                      {trigger.agentName}
                      {trigger.pipeline && <span className="ml-1.5">filter: {trigger.pipeline}</span>}
                    </p>
                  </div>
                  <button
                  ...
                  <button onClick={() => toggleTrigger(trigger.id, 'enabled')} className="text-slate-500 hover:text-white transition-colors">
                    {trigger.enabled ? <ToggleRight className="w-5 h-5 text-blue-400" /> : <ToggleLeft className="w-5 h-5" />}
                  </button>
      ...
      <section className="rounded-xl border border-white/8 bg-white/[0.02] p-4 space-y-2">
        ...
        <p className="text-[11px] text-slate-500 leading-relaxed">
          ...
          governance pipeline. Write actions like <span className="text-slate-400 font-medium">send_email</span>,
          <span className="text-slate-400 font-medium"> create_event</span>, and
          <span className="text-slate-400 font-medium"> share_file</span> check action policies,
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          ...
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] text-slate-400 font-medium">
            <Shield className="w-3 h-3 text-blue-400" /> Policy check
          </span>
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] text-slate-400 font-medium">
