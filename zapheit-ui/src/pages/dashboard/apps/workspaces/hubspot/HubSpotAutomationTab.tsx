...
type TriggerType = 'contact_created' | 'deal_stage_changed' | 'form_submitted' | 'lead_scored';
...
const TRIGGER_META: Record<TriggerType, { label: string; description: string; Icon: typeof Bell }> = {
  contact_created:     { label: 'Contact created',     description: 'Agent enriches or qualifies new contacts',           Icon: UserPlus },
  deal_stage_changed:  { label: 'Deal stage changed',  description: 'Agent acts when a deal moves pipeline stages',       Icon: DollarSign },
  form_submitted:      { label: 'Form submitted',      description: 'Agent responds to inbound form submissions',         Icon: Users },
  lead_scored:         { label: 'Lead scored',          description: 'Agent routes high-score leads to sales reps',         Icon: Sparkles },
};
...
export function HubSpotAutomationTab() {
  ...
  const [newTriggerType, setNewTriggerType] = useState<TriggerType>('contact_created');
  ...
  return (
    <div className="p-5 space-y-8 max-w-2xl">
      ...
      <section className="space-y-3">
        ...
        <div className="relative">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-orange-500/30 pr-8"
            >
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder={'e.g. "Create a contact for jane@acme.com with lifecycle stage lead"'}
            className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-orange-500/30"
            disabled={executing}
          />
          <button
            onClick={() => void handleExecuteCommand()}
            disabled={!command.trim() || !selectedAgent || executing}
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-orange-500 hover:bg-orange-400 text-white text-xs font-semibold transition-colors disabled:opacity-30 disabled:cursor-not-allowed shrink-0"
          >
            {executing ? (
        ...
        <div className="space-y-1">
          ...
          {[
            'Create a contact for jane@acme.com, VP of Sales at Acme Inc',
            'Move deal "Enterprise SaaS" to Contract Sent stage',
            'Add a note to contact #123: "Had great demo call, follow up Friday"',
            'Search for all leads from the tech industry',
          ].map((ex) => (
            <button
              key={ex}
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Configure agents to automatically respond to HubSpot events. All actions go through governance.
        </p>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full appearance-none px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-orange-500/30"
              >
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. default or leave blank for all pipelines"
                className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-orange-500/30"
              />
            </div>
        ...
        {triggers.length === 0 && !showAddTrigger ? (
          <div className="text-center py-8">
            ...
            <p className="text-[10px] text-slate-600 mt-1">Add a trigger to let agents respond to HubSpot events automatically.</p>
          </div>
        ) : (
      ...
      <section className="rounded-xl border border-white/8 bg-white/[0.02] p-4 space-y-2">
        ...
        <p className="text-[11px] text-slate-500 leading-relaxed">
          ...
          governance pipeline. Write actions like <span className="text-slate-400 font-medium">create_contact</span>,
          <span className="text-slate-400 font-medium"> create_deal</span>, and
          <span className="text-slate-400 font-medium"> update_contact</span> check action policies,
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          ...
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] text-slate-400 font-medium">
            <Shield className="w-3 h-3 text-orange-400" /> Policy check
          </span>
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md border border-white/10 bg-white/[0.03] text-slate-400 font-medium">
