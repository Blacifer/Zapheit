...
const CONTROL_PLANE_URL = env('ZAPHEIT_CONTROL_PLANE_URL', 'http://localhost:3001')!.replace(/\/+$/, '');
const RUNTIME_ID = env('ZAPHEIT_RUNTIME_ID', '')!;
const ENROLLMENT_TOKEN = env('ZAPHEIT_ENROLLMENT_TOKEN', '')!;
const RUNTIME_SECRET_ENV = env('ZAPHEIT_RUNTIME_SECRET', '')!;
const RUNTIME_SECRET_FILE = env('ZAPHEIT_RUNTIME_SECRET_FILE', '')!;
const API_KEY = env('ZAPHEIT_API_KEY', '')!;
const GATEWAY_URL = env('ZAPHEIT_GATEWAY_URL', `${CONTROL_PLANE_URL}/v1`)!.replace(/\/+$/, '');
const POLL_INTERVAL_MS = Math.max(750, Number(env('ZAPHEIT_POLL_INTERVAL_MS', '2000')));
const HEARTBEAT_INTERVAL_MS = Math.max(5000, Number(env('ZAPHEIT_HEARTBEAT_INTERVAL_MS', '15000')));
const SCHEDULER_INTERVAL_MS = Math.max(30_000, Number(env('ZAPHEIT_SCHEDULER_INTERVAL_MS', '60000')));
const DEFAULT_MODEL = env('ZAPHEIT_MODEL', 'openai/gpt-4o-mini')!;
const STREAM_MODE = env('ZAPHEIT_STREAM_MODE', 'poll')!; // poll | stream
const WEBHOOK_ALLOWLIST = env('ZAPHEIT_WEBHOOK_ALLOWLIST', '')!;

if (!RUNTIME_ID) {
  console.error('Missing required env: ZAPHEIT_RUNTIME_ID');
  process.exit(1);
}

if (!API_KEY) {
  console.error('Missing required env: ZAPHEIT_API_KEY (create it via Zapheit → Connect Agent wizard)');
  process.exit(1);
}
...
const GCP_SECRET_NAME = 'ZAPHEIT_RUNTIME_SECRET';
...
async function enroll(): Promise<void> {
  ...
  if (!ENROLLMENT_TOKEN) {
    throw new Error('Missing required env: ZAPHEIT_ENROLLMENT_TOKEN (needed for first-time enrollment)');
  }
...
async function runChatTurn(job: JobRow) {
  ...
  const messages = Array.isArray(input.messages) ? input.messages : [
    ...
    { role: 'user', content: 'Hello from Zapheit runtime.' },
  ];
