...
export function buildOAuthUrl(appId: string, state: string, redirectUri: string): string | null {
  ...
  switch (appId) {
    ...
    case 'google-workspace':
      ...
      return `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=${enc(process.env.GOOGLE_CLIENT_ID)}&redirect_uri=${enc(redirectUri)}&scope=${enc('openid email profile https://www.googleapis.com/auth/gmail.send https://www.googleapis.com/auth/calendar https://www.googleapis.com/auth/admin.directory.user.readonly https://www.googleapis.com/auth/drive.readonly https://www.googleapis.com/auth/documents')}&access_type=offline&prompt=consent&state=${state}`;
    case 'microsoft-365':
