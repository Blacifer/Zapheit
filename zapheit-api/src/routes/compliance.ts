...
async function streamCompliancePdf(
  ...
  try {
    ...
    const filename = `zapheit-compliance-report-${Date.now()}.pdf`;
    ...
    doc.fillColor('#ffffff').fontSize(22).font('Helvetica-Bold').text('Zapheit Compliance Report', 50, 40);
    ...
    doc.moveDown(0.5);
    doc.fillColor(GRAY).fontSize(9)
      .text('This report was automatically generated by the Zapheit AI governance platform. It is intended as supporting evidence for compliance and audit purposes. Zapheit does not certify regulatory compliance — consult your compliance officer for formal assessments.');
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc.fillColor(GRAY).fontSize(8)
        .text(`Zapheit Compliance Report  ·  ${orgName}  ·  Confidential  ·  Page ${i + 1} of ${pageCount}`, 50, doc.page.height - 40, { align: 'center', width: pageW });
    }
...
router.get('/iso42001.pdf', async (req: Request, res: Response) => {
  ...
  try {
    ...
    doc.moveDown(0.5);
    doc.fillColor(GRAY).fontSize(9).font('Helvetica').text('ISO 42001 is the international standard for AI Management Systems. The table below maps each clause to Zapheit platform controls.', 50, undefined, { width: pageW });
    doc.moveDown(0.8);
    ...
    for (let i = 0; i < pageCount; i++) {
      doc.switchToPage(i);
      doc.fillColor(GRAY).fontSize(8)
        .text(`Zapheit AI Management System  ·  ISO/IEC 42001:2023 Evidence Package  ·  ${orgName}  ·  Page ${i + 1} of ${pageCount}`, 50, doc.page.height - 40, { align: 'center', width: pageW });
    }
...
router.get('/data-export.zip', requirePermission('compliance.export'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    archive.append(JSON.stringify({
      ...
      generator: 'Zapheit AI Governance Platform',
...
router.delete('/subject-erasure', requirePermission('compliance.export'), async (req: Request, res: Response) => {
  try {
    ...
    const receipt_signature = crypto
      .createHash('sha256')
      .update(`${process.env.ERASURE_SIGNING_SALT || 'zapheit'}:${receiptPayload}`)
      .digest('hex');
