...
type TriggerType = 'push' | 'pr_opened' | 'issue_created' | 'review_requested';
...
const TRIGGER_META: Record<TriggerType, { label: string; description: string; Icon: typeof Bell }> = {
  push:               { label: 'Push to branch',      description: 'Agent reviews commits when code is pushed',          Icon: GitCommit },
  pr_opened:          { label: 'PR opened',            description: 'Agent triages or reviews new pull requests',         Icon: GitPullRequest },
  issue_created:      { label: 'Issue created',        description: 'Agent triages or responds to new issues',            Icon: CircleDot },
  review_requested:   { label: 'Review requested',     description: 'Agent performs code review when assigned as reviewer', Icon: Eye },
};
...
export function GitHubAutomationTab() {
  ...
  const [newTriggerType, setNewTriggerType] = useState<TriggerType>('push');
  const [newTriggerAgent, setNewTriggerAgent] = useState('');
  const [newTriggerRepo, setNewTriggerRepo] = useState('');
  ...
  }, [newTriggerAgent, newTriggerType, newTriggerRepo, agents]);
  ...
  return (
    <div className="p-5 space-y-8 max-w-2xl">
      ...
      <section className="space-y-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder={'e.g. "Create an issue for login page crash in org/repo"'}
        ...
        <div className="space-y-1">
          ...
          {[
            'Create an issue in org/repo: "API rate limiting needs improvement"',
            'Open a PR to merge feature/auth into main',
            'Comment on issue #42: "Fixed in commit abc123"',
            'Merge pull request #15 in org/repo after checks pass',
          ].map((ex) => (
            <button
              key={ex}
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Configure agents to automatically respond to GitHub events. All actions go through governance.
        </p>
        ...
        {showAddTrigger && (
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
            ...
            {/* Repo (optional) */}
            <div>
              <label className="text-[10px] text-slate-500 uppercase tracking-wider font-medium block mb-1">Repository (optional)</label>
              <input
                type="text"
                value={newTriggerRepo}
                onChange={(e) => setNewTriggerRepo(e.target.value)}
                placeholder="e.g. org/repo or leave blank for all repos"
        ...
        {triggers.length === 0 && !showAddTrigger ? (
          <div className="text-center py-8">
            ...
            <p className="text-[10px] text-slate-600 mt-1">Add a trigger to let agents respond to GitHub events automatically.</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500">
                      ...
                      {trigger.agentName}
                      {trigger.repo && <span className="ml-1.5">in {trigger.repo}</span>}
                    </p>
                  </div>
      ...
      <section className="rounded-xl border border-white/8 bg-white/[0.02] p-4 space-y-2">
        ...
        <p className="text-[11px] text-slate-500 leading-relaxed">
          ...
          <span className="text-slate-400 font-medium"> create_pull</span>, and
          <span className="text-slate-400 font-medium"> merge_pull</span> check action policies,
