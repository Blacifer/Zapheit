...
type TriggerType = 'new_message' | 'mention' | 'reaction' | 'channel_join';
...
const TRIGGER_META: Record<TriggerType, { label: string; description: string; Icon: typeof Bell }> = {
  new_message:  { label: 'New message',    description: 'Agent responds when a message is posted',  Icon: MessageCircle },
  mention:      { label: 'Agent mentioned', description: 'Agent responds when @mentioned',           Icon: Bell },
  reaction:     { label: 'Reaction added',  description: 'Agent reacts to emoji reactions',          Icon: Sparkles },
  channel_join: { label: 'Channel join',    description: 'Agent greets when someone joins a channel', Icon: Plus },
};
...
export function SlackAutomationTab() {
  ...
  const [newTriggerType, setNewTriggerType] = useState<TriggerType>('mention');
  const [newTriggerAgent, setNewTriggerAgent] = useState('');
  const [newTriggerChannel, setNewTriggerChannel] = useState('');
  ...
  const handleAddTrigger = useCallback(() => {
    ...
    const trigger: EventTrigger = {
      ...
      type: newTriggerType,
      channel: newTriggerChannel || undefined,
      enabled: true,
      proposeApprove: true, // default to propose-approve mode
    };
    setTriggers((prev) => [...prev, trigger]);
    setShowAddTrigger(false);
    setNewTriggerAgent('');
    setNewTriggerChannel('');
    toast.success(`Trigger added: ${TRIGGER_META[newTriggerType].label}`);
  }, [newTriggerAgent, newTriggerType, newTriggerChannel, agents]);
  ...
  return (
    <div className="p-5 space-y-8 max-w-2xl">
      ...
      <section className="space-y-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder={'e.g. "Post a summary of today\'s incidents to #general"'}
        ...
        <div className="space-y-1">
          ...
          {[
            'Post the daily standup summary to #engineering',
            'Send a reminder about the deadline to #project-alpha',
            'Set the topic of #general to "Q2 planning week"',
          ].map((ex) => (
            <button
              key={ex}
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Configure agents to automatically respond to Slack events. All actions go through governance.
        </p>
        ...
        {showAddTrigger && (
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
            ...
            {/* Channel (optional) */}
            <div>
              <label className="text-[10px] text-slate-500 uppercase tracking-wider font-medium block mb-1">Channel (optional)</label>
              <input
                type="text"
                value={newTriggerChannel}
                onChange={(e) => setNewTriggerChannel(e.target.value)}
                placeholder="#channel-name or leave blank for all"
        ...
        {triggers.length === 0 && !showAddTrigger ? (
          <div className="text-center py-8">
            ...
            <p className="text-[10px] text-slate-600 mt-1">Add a trigger to let agents respond to Slack events automatically.</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500">
                      ...
                      {trigger.agentName}
                      {trigger.channel && <span className="ml-1.5">in {trigger.channel}</span>}
                    </p>
                  </div>
      ...
      <section className="rounded-xl border border-white/8 bg-white/[0.02] p-4 space-y-2">
        ...
        <p className="text-[11px] text-slate-500 leading-relaxed">
          ...
          governance pipeline. Write actions check <span className="text-slate-400 font-medium">action policies</span>,
          may require <span className="text-slate-400 font-medium">human approval</span>, and are logged
          in <span className="text-slate-400 font-medium">governed actions</span> with a full audit trail.
        </p>
